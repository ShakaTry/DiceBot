...
class TestProvablyFairGenerator:
    ...
    def test_initialization(self) -> None:
        ...
        generator: ProvablyFairGenerator = ProvablyFairGenerator()
    ...
    def test_initialization_with_seeds(self) -> None:
        ...
        generator: ProvablyFairGenerator = ProvablyFairGenerator(server_seed, client_seed)
    ...
    def test_server_seed_hash(self) -> None:
        ...
        server_seed: str = "test_server_seed"
        generator: ProvablyFairGenerator = ProvablyFairGenerator(server_seed, "client")

        expected_hash: str = hashlib.sha256(server_seed.encode()).hexdigest()
    ...
    def test_set_client_seed(self) -> None:
        ...
        generator: ProvablyFairGenerator = ProvablyFairGenerator()
        new_client_seed: str = "my_custom_seed"
    ...
    def test_generate_dice_result(self) -> None:
        ...
        generator: ProvablyFairGenerator = ProvablyFairGenerator("server123", "client456")

        # Premier résultat
        result1: float = generator.generate_dice_result()
        assert 0.0 <= result1 <= 99.99
        assert generator.current_seeds.nonce == 1

        # Deuxième résultat (différent)
        result2: float = generator.generate_dice_result()
    ...
    def test_reproducible_results(self) -> None:
        ...
        server_seed: str = "fixed_server"
        client_seed: str = "fixed_client"

        # Premier générateur
        gen1: ProvablyFairGenerator = ProvablyFairGenerator(server_seed, client_seed)
        results1: list[float] = [gen1.generate_dice_result() for _ in range(5)]

        # Deuxième générateur avec mêmes seeds
        gen2: ProvablyFairGenerator = ProvablyFairGenerator(server_seed, client_seed)
        results2: list[float] = [gen2.generate_dice_result() for _ in range(5)]
    ...
    def test_rotate_seeds(self) -> None:
        ...
        old_server: str = generator.current_seeds.server_seed
        old_client: str = generator.current_seeds.client_seed
        old_nonce: int = generator.current_seeds.nonce
    ...
    def test_verify_result(self) -> None:
        ...
        result: float = generator.generate_dice_result()
        nonce: int = generator.current_seeds.nonce - 1  # Le nonce a été incrémenté

        # Vérification correcte
        is_valid: bool = generator.verify_result("server123", "client456", nonce, result)
        assert is_valid

        # Vérification incorrecte
        is_invalid: bool = generator.verify_result("wrong_server", "client456", nonce, result)
    ...
    def test_bitsler_algorithm_compatibility(self) -> None:
        ...
        server_seed: str = "e6bbf5eda32e178e78a2c8e73b4b8bea1c17e01ac5b8e5c0d42d2a29f4b76eb7"
        client_seed: str = "test_client"
        nonce: int = 0

        # Calcul manuel selon l'algorithme Bitsler
        message: str = f"{client_seed},{nonce}"
        seed_hash: str = hmac.new(
        ...
        while offset + 5 <= len(seed_hash):
            hex_chunk: str = seed_hash[offset : offset + 5]
            number = int(hex_chunk, 16)
            if number <= 999999:
                break
            offset += 5

        expected_result: float = (number % 10000) / 100

        # Test avec notre générateur
        generator: ProvablyFairGenerator = ProvablyFairGenerator(server_seed, client_seed)
        actual_result: float = generator.generate_dice_result()
...
class TestBitslerVerifier:
    ...
    def test_verify_dice_result(self) -> None:
        ...
        server_seed: str = "test_server"
        client_seed: str = "test_client"
        nonce: int = 0

        # Générer un résultat connu
        generator: ProvablyFairGenerator = ProvablyFairGenerator(server_seed, client_seed)
        expected_result: float = generator.generate_dice_result()

        # Vérifier
        verification: dict[str, Any] = BitslerVerifier.verify_dice_result(
    ...
    def test_batch_verify(self) -> None:
        ...
        server_seed: str = "batch_server"
        client_seed: str = "batch_client"
        ...
        # Vérification batch
        batch_result: dict[str, Any] = BitslerVerifier.batch_verify(results)
...
class TestDiceGameProvablyFair:
    ...
    def test_dice_game_legacy_mode(self) -> None:
        """Test le mode legacy."""
        game: DiceGame = DiceGame(use_provably_fair=False, seed=12345)
    ...
    def test_roll_with_provably_fair(self) -> None:
        ...
        game: DiceGame = DiceGame(server_seed="test_server", client_seed="test_client")
    ...
    def test_set_client_seed(self) -> None:
        """Test le changement de client seed."""
        game: DiceGame = DiceGame(client_seed="initial")

        game.set_client_seed("new_seed")

        seed_info: dict[str, Any] = game.get_current_seed_info() or {}
    ...
    def test_set_client_seed_legacy_mode_error(self) -> None:
        ...
        game: DiceGame = DiceGame(use_provably_fair=False)
    ...
    def test_verify_result(self) -> None:
        ...
        game: DiceGame = DiceGame(server_seed="verify_server", client_seed="verify_client")
    ...
    def test_reproducible_dice_game_results(self) -> None:
        ...
        server_seed: str = "repro_server"
        client_seed: str = "repro_client"

        # Premier jeu
        game1: DiceGame = DiceGame(server_seed=server_seed, client_seed=client_seed)
        results1: list[float] = [game1.roll(Decimal("1"), 2.0).roll for _ in range(3)]

        # Deuxième jeu avec mêmes seeds
        game2: DiceGame = DiceGame(server_seed=server_seed, client_seed=client_seed)
        results2: list[float] = [game2.roll(Decimal("1"), 2.0).roll for _ in range(3)]
